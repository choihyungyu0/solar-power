...
> 개발자 `aiSimulationResult` 를 받아 **주택 유형별 보조금 + 대출**을 결합한 분석 보고서(HTML + 구조화 데이터)를 생성하는 에이전트입니다.
...
- 처리: ① 건물 용도/주소 기준으로 **단독주택 vs 아파트/공동주택 제도 분기** → ② 자부담 → ③ 대출(목업 가정) → ④ 실투자금·회수기간 재계산.
...
샘플(화성 67.8kW)에서 **입력값 88,200 kWh/년이 ML 발전량 예측 24,131 kWh/년으로 덮어써지던 문제**가 확인되었습니다. `ml_simulation_model.predict_generation()`은 이제 외부 API/상위 산식의 `annualGenerationKwh`가 있으면 그 값을 우선 사용하고, seed data 기반 ML 예측이 용량 대비 과소 산출되면 보수적 물리 하한 guardrail을 적용합니다. 따라서 보고서 회수기간도 보정된 발전량을 기준으로 계산됩니다.
