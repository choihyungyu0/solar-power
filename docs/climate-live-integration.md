...
3. `selectBuld.do`를 5초 timeout, 1회 시도로 호출한다.
...
전체 `/api/climate-rooftop-analysis` 처리는 Vercel production timeout에 의존하지 않고 내부적으로 25초 제한을 둔다. 단계별 제한은 다음처럼 짧게 유지한다.

- `selectBuld.do`: 5초, 1회 시도
- WFS 건물 메타데이터: 5초
- `selectSunList.do`: 12초
- `selectBuldInfo.do`: 4초
- `selectRuleList.do`: 4초
- `pv/analysis`: 8초
...
`pv/analysis`가 실패하거나 지연되면 이미 만든 bundle/panel geometry는 버리지 않는다. API는 데모 경제성 산식으로 `pv_analysis_output`을 채우고 `diagnostics.pvAnalysisStatus`와 `diagnostics.fallbackReason`에 fallback 사유를 남긴다.
