...
Vercel production timeout을 피하기 위해 기본 라이브 분석은 `mode: "fast"`, `includePvAnalysis: false`로 실행한다. 이 요청은 shading cell과 패널 GeoJSON까지만 빠르게 반환하고, 발전량/경제성 계산은 프론트엔드가 `/api/pv-analysis`로 별도 요청한다.
...
3. fast mode에서는 `selectBuld.do`를 3초 timeout, 1회 시도로 호출한다.
...
8. fast mode에서는 셀 수가 700개를 넘으면 결정적 샘플링으로 700개만 분석하고 `maxCellsApplied = true`를 반환한다.
...
기본 fast mode의 `/api/climate-rooftop-analysis` 처리는 Vercel production timeout에 의존하지 않고 내부적으로 15초 제한을 둔다. 단계별 제한은 다음처럼 짧게 유지한다.

- 전체 fast 요청: 15초
- `selectBuld.do`: 3초, 1회 시도
- `selectSunList.do`: 9초
- fast mode 셀 상한: 700개
...
`selectSunList.do`가 실패하거나 지연되면 shading score가 없으므로 API는 `ok:false`, `fallbackRecommended:true`를 반환한다. 이때 메시지는 `climate.gg 음영 분석 응답 지연으로 건물 footprint 기반 자체 배치를 유지합니다.`이며, 프론트엔드는 기존 건물 footprint 기반 자체 패널 grid fallback을 유지한다.
